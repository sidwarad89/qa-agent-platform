    ...
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-950 via-indigo-950 to-slate-900 px-4">
      <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-xl p-8 w-full max-w-sm flex flex-col gap-4">
        <div className="text-center mb-1">
          ...
          <p className="text-sm text-slate-500 mt-1">Join QA Agent Builder</p>
        </div>
        ...
        <div className="flex flex-col gap-1">
          ...
          <input type="email" className="border border-slate-300 rounded-lg px-3 py-2.5" value={email} onChange={(e) => setEmail(e.target.value)} required />
        </div>

        <div className="flex flex-col gap-1">
          <label className="text-sm font-medium text-slate-600">Password</label>
          <input type="password" className="border border-slate-300 rounded-lg px-3 py-2.5" value={password} onChange={(e) => setPassword(e.target.value)} required minLength={6} />
        </div>
        ...
        <p className="text-sm text-slate-500 text-center">
          ...
          <button type="button" onClick={onSwitchToSignIn} className="text-indigo-600 font-medium">Sign in</button>
        </p>
      </form>
    </div>
  )
